...
my $sth = $dbh->prepare("SELECT user_name, homework, point, on_time 
                  ...
                  ORDER BY webprog5_sofonov_student.user_name, homework") 
...
my $template = HTML::Template->new(filename => 'sofonov_results.html', die_on_bad_params => 0);
